...
BOOL  CIniFile::Create(const CString & strFileName)
{
	...
	{
		AfxMessageBox("配置文件不存在，请将配置文件放置在可执行文件目录下",MB_OK);
		return bFileExsit;
	}
	CString strFileLine;
...
BOOL CIniFile::GetVarShort(const CString & strSection,const CString & strVarName,short & iValue)
{
	CString  strReturnVar;
	if(GetVar(strSection,strVarName,strReturnVar))
	{
...
BOOL CIniFile::GetVarFloat(const CString & strSection,const CString & strVarName,double & fValue)
{
	CString  strReturnVar;
	if(GetVar(strSection,strVarName,strReturnVar))
	{
		strReturnVar.TrimLeft();
		int iLen = strReturnVar.GetLength();
		fValue = atof(strReturnVar.GetBuffer(iLen));
		return TRUE;
	}
	return TRUE;
}
...
BOOL CIniFile::SetVarFloat(const CString & strSection,const CString & strVarName,const double & fValue,const int iType)
{
	CString strVar;
	strVar.Format("%lf",fValue);
	return (SetVar(strSection,strVarName,strVar,iType));
}
